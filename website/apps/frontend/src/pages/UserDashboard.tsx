...
export const UserDashboard = () => {
  ...
  // Track MCQ set count separately to add to total_sets
  ...
  const fetchStoredMaterials = async () => {
    try {
      ...
      // Track MCQ count to add onto stats.total_sets
      ...
      const [flashEnriched, mcqEnriched] = await Promise.all([
        Promise.all((flashData.flashcard_sets || []).map(async (set: any) => {
          ...
          return { id: set.id, title: set.title, content: `${set.flashcards?.length || 0} flashcards`, date: new Date(set.created_at).toLocaleDateString(), createdAt: set.created_at, ...status };
        })),
        Promise.all(mcqSets.map(async (set: any) => {
          const results = await fetchResultsForSet(set.id);
          const status = getMaterialStatus(results);
          return { id: set.id, title: set.title, content: `${set.questions?.length || 0} questions`, date: new Date(set.created_at).toLocaleDateString(), createdAt: set.created_at, ...status };
        })),
      ]);
  ...
  const pieData = {
    labels: ["Proficient", "Non-Proficient", "Due for Review", "Unattempted"],
    datasets: [{
  ...
  // Total sets = flashcard sets (from API) + MCQ sets
      ...
      <div className="min-h-screen text-white" style={{ background: "#07121d", fontFamily: "'Sora', 'Poppins', sans-serif" }}>
        ...
        <header className="relative flex items-center justify-between px-6 py-4 border-b border-white/[0.06] overflow-hidden"
          ...
          <div className="relative z-10 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-white tracking-tight">Learning Dashboard</h1>
              <p className="text-xs text-slate-500">Track your progress and insights</p>
            </div>
          </div>
        ...
        <main className="max-w-6xl mx-auto px-6 py-8 space-y-7">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard title="Total Study Sets" value={totalSets} icon={<Layers size={18} />} />
            <StatCard title="Quizzes Completed" value={stats.total_quizzes} icon={<Brain size={18} />} accent="#818cf8" />
            <StatCard title="Avg Performance" value={`${stats.avg_score}%`} icon={<TrendingUp size={18} />} accent="#22c55e" />
            <StatCard title="Current Streak" value={`${streak}d`} icon={<Flame size={18} />} accent="#f59e0b" />
          </div>

          {/* ── CHARTS ── */}
          <div className="rounded-2xl border border-white/[0.07] overflow-hidden" style={{ background: "#0b1b2b" }}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
              <h2 className="text-base font-bold text-white tracking-tight">Study Analytics</h2>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-px" style={{ background: "rgba(255,255,255,0.04)" }}>
              ...
              <div className="flex flex-col p-6 gap-4" style={{ background: "#0b1b2b" }}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[11px] text-slate-500 uppercase tracking-widest font-semibold">Activity Trend</p>
                    <h3 className="text-sm font-bold text-white mt-0.5">Review Frequency</h3>
                  </div>
              ...
              <div className="flex flex-col p-6 gap-4" style={{ background: "#0b1b2b" }}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[11px] text-slate-500 uppercase tracking-widest font-semibold">Performance Split</p>
                    <h3 className="text-sm font-bold text-white mt-0.5">Mastery Breakdown</h3>
                  </div>
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "rgba(220,101,5,0.1)", color: "#dc6505" }}>
                    <PieChart size={16} />
                  </div>
          ...
          <div className="rounded-2xl border border-white/[0.07] overflow-hidden" style={{ background: "#0b1b2b" }}>
            ...
            <div className="divide-y divide-white/[0.05]">
              {filteredRecent.length === 0 ? (
                <p className="text-slate-500 text-sm text-center py-10">No results found.</p>
                ...
                return (
                  <div key={i} className="flex flex-wrap items-center gap-3 px-6 py-3.5 transition-colors hover:bg-white/[0.02]">
                    {/* Title */}
                    <span className="flex-1 min-w-0 text-sm font-medium text-slate-200 truncate">{item.title || "Untitled"}</span>
